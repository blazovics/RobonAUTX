...
void RemoteCentralController::EventReceived(Event &event)
{
    ...
    case Device_ICentralController + Event_UpdateCheckpointState:
    {
        ...
        break;
    case Device_ICentralController + Event_RevertCheckpointState:
    {
        quint32 first = event.extractQuint32FromRawData();
        localController->RevertCheckpointState(first);
    }
        break;
...
void RemoteCentralController::RevertCheckpointState(quint32 checkpointID)
{
    Event event(Device_ICentralController + Event_RevertCheckpointState);
    event.insertQuint32(checkpointID);
    sendEvent(event);
}
